...
profile_rank_df <- product_emission |>
  ...
  rename("grouping_emission" = "benchmark") |>
  distinct() |>
...
avg_profile_ranking_df <- profile_rank_df |>
  mutate(sum_profile_ranking = sum(.data$profile_ranking, na.rm = TRUE), .by = "grouping_emission") |>
  mutate(distinct_products_per_benchmark = n_distinct(.data$ep_product, na.rm = TRUE), .by = "grouping_emission") |>
...
avg_reduction_targets_df <- reduction_targets_df |>
  mutate(sum_reduction_targets = sum(.data$reduction_targets, na.rm = TRUE), .by = "grouping_sector") |>
  mutate(distinct_products_per_scenario_year = n_distinct(.data$ep_product, na.rm = TRUE), .by = "grouping_sector") |>
...
```{r, results='asis', echo=FALSE}
kable(companies_atleast_one_matched_product |> head(10))
```
...
```{r, results='asis', echo=FALSE}
kable(result |> head(10))
```

```{r, echo=FALSE}
average_result <- sum(result$emission_category_NA, na.rm = TRUE) / n_distinct(result$companies_id)
```
